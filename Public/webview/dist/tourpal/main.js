...
define('src/js/core',['lib/zepto','src/web/main','lib/zepto.history','src/js/pageStorage'],function(zepto,blend,his){
 ...
 * 支持translate3d
*/
define('src/js/sliders',['src/js/core'], function(core){
	/*
	从Modernizr.js中移植出来
	*/
	var css3 = {},
...
define('entry/tourpal/js/hotcity',['src/js/core', 'lib/artTemplate', 'src/js/city', 'src/js/PluginManager', 'entry/tourpal/js/tourpal', 'src/js/sliders'], function(core, template, city, PluginManager, tourpal){
    //路径配置
    var path = {
...
define('entry/tourpal/js/postlist',['src/js/core', 'src/js/dialog', 'entry/tourpal/js/tourpal', 'entry/tourpal/js/scrollLoad', 'entry/tourpal/js/picview'], function(core, dialog, tourpal, ScrollLoad, picview){
    ...
    core.onrender('postlist',function(dom){
        ...
        $postlist.on('click', '.join', function(){
                    ...
                    success : function(data){
                        if(data.code == 0){
                            ...
                            dialog.confirm({
                                content : '求同行前请完善用户信息！',
                                success : function(){
                                    var params = $.param({redirect : 'postlist', city : city});
                                    core.addPager({
                                      id:'mycard',
                                      url:'mycard?' + params,
                                      active:true
            ...
            //检测是否登录
            if(!tourpal.isLogin()){
                tourpal.login(function(){sl.reload();});
                return false;
            } 
            //同行或取消同行
	...
	$.extend($.fn, {
		calendar: function(options) {
			...
			var _buildUi = function() {
				...
				// .appendTo('body');
				pageCtrl.header = $('<header class="bar bar-nav"><nav><h1 class="title">出发日期</h1></nav></header>').appendTo(pageWrap);
       ...
       var upload = function (settings) {  
                ...
                window.successPhoto = function(data){
                    ...
                    if(data.action == 0){
                        for(var i = 0; i < data.num; i++){
                            $imgList.prepend('<li ><img src="' + options.loadingImg + '" class="loading"><span class="del">&times;</span></li>');
                        }
                    }
                    //上传成功
                    if(data.action == 1){
                        var picId = data.imgUrl.substr(data.imgUrl.lastIndexOf('/')+1).replace('.jpg', '');
                        $imgList.find('.loading').eq(0).removeClass('loading').attr('src', data.imgUrl).parent().attr('data-picid', picId);

                    }
                }
                $('.add-btn').on('click', function(){ 
                    ElongApp.showPhoto('successPhoto', JSON.stringify({maxNum : 4 - $imgList.find('li').length }));
                });
            }
            ...
            var bindEvent = function(){
                $('.add-btn').on('change', '.watch-change', function(){  
                    ...
                    addInput();
                    $imgList.prepend('<li data-id="' + formId +'"><img src="' + options.loadingImg + '" ><span class="del">&times;</span></li>');
...
define('src/js/comment',['src/js/core'],function(core) {
	$.extend($.fn, {
		comment : function(){
			...
			var comment = {
				...
				init : function(){
					...
					$(".confirm", page).on("click", function(){
						var text = page.find("textarea").val();
						that.postComment(text);
					});
				},
				beforeShow : function(){
					var textarea = this.page.find("textarea");
					setTimeout(function(){
						textarea.focus();
					}, 500);
				},
...
define('entry/tourpal/js/postdetail',['src/js/core', 'src/js/dialog', 'entry/tourpal/js/tourpal', 'entry/tourpal/js/picview', 'lib/artTemplate', 'src/js/PluginManager', 'src/js/comment'], function(core, dialog, tourpal, picview, template, PluginManager){
                ...
                success : function(data){
                    if(data.code == 0){
                        if(data.data){
                            ...
                            //载入城市选择插件
                            PluginManager.add("comment", $('.comment').comment());
                        }
                    }
                }
            });
